...
import {getAllDynamic, getDynamicDetail, getTaskDetail, getUserInfo, sendDynamicComment} from "@/api";
  ...
  for (let dynamic of dynamics.value) {
    ...
    if (dynamic.id === send_dynamic_id.value) {
      ...
      dynamic.children.push({
        id: "",
        commenterName: <string>user?.nickname,
    ...
    <van-popup v-model:show="showInput"
      ...
      <van-cell-group inset>
        <van-field
            ...
            show-word-limit
        />
        <div @click="input_comment" :class="comment_input_words === '' ? 'send_button' : 'send_button_active'">发布回答</div>
      </van-cell-group>
    </van-popup>
  </div>
</template>
